...
> ⚠️ **本文件部分内容早于「来源分段」改造,照抄命令会报错。**
> 要做全量部署请先读 **[../README.md](../README.md) 的「全量部署」与「注意事项」** ——
> 那两份是最新的。本文件里**仍然有效**的是各步的**业务规则**(§6 流水线各步明细、
> §7 关键业务规则、§9 与旧表对比的预期差异、§10 ChEBI 参考库维护)。
> **以下内容已过期,不要照抄:**
>
> | 过期处 | 现在是什么 |
> |---|---|
...
>
> 另:§9 的基线是 2026-08 的 1,524 酶规模,现在的库是 95,869 酶。

> 本文件是 `update_tool` 全新数据工作流的**完整操作与维护手册**(分段改造前版本)。
